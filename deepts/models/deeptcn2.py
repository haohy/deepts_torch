...
from deepts.layers import DeepTCN2Layer

def DeepTCN2(n_back, n_fore, dilation_list, conv_filter, conv_ksize, dilation_depth, 
            n_repeat, static_dim, dynamic_dim):
    input_ts = Input(shape=[n_back,])
    input_static_emb = Input(shape=[n_back+n_fore, static_dim])
    input_dynamic_emb = Input(shape=[n_back+n_fore, dynamic_dim])
    tcn_layer = DeepTCN2Layer(n_back, n_fore, dilation_list, conv_filter, conv_ksize, 
