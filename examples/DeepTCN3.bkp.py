...
def TSF_DeepTCN(config_model, config_dataset, model_name, ds_name, criterion_type, 
    ...
    for epoch in range(1, epochs+1):
        ...
        nd_valid, smape_valid, nrmse_valid, mae_valid, mse_valid, ms_dtw_valid, y_pred_valid = evaluate(model, x_valid, y_valid, multi_scale_dtw_loss, scale_all, gamma, device, False)
        y_valid = y_valid.to(device)
        if criterion_type == 'ms_dtwi':
        ...
        logger.add_scalars('{}/ms_dtw_valid'.format(logger_note), {'ms_dtw_valid':ms_dtw_valid}, epoch) 
        print("Epoch {}, Train Loss {:.4f}, Valid ND {:.4f}, Valid SMAPE {:.4f}, Valid NRMSE {:.4f}, Valid loss {:.4f}, Valid MS-DTW {:.4f}"\
            .format(epoch, loss/len(train_dataloader), nd_valid, smape_valid, nrmse_valid, loss_val.item(), ms_dtw_valid))
        ...
        if loss_val < loss_val_best:
            ...
            # nd_test, smape_test, nrmse_test, mae_test, mse_test, _, y_pred_test = evaluate(model, x_test, y_test, multi_scale_dtw_loss, scale_all, gamma, device)
            # y_pred_test_best = y_pred_test
            # print("Epoch {}, Train Loss {:.4f}, Test ND {:.4f}, Test SMAPE {:.4f}, Test NRMSE {:.4f}, Test loss {:.4f}, Test ms-dtw {:.4f}"\
            # .format(epoch, loss/len(train_dataloader), nd_test, smape_test, nrmse_test, loss_val.item(), ms_dtw_test))

    nd_test, smape_test, nrmse_test, mae_test, mse_test, ms_dtw_test, y_pred_test_best = evaluate(model_best, x_test, y_test, multi_scale_dtw_loss, scale_all, gamma, device, True)
    print("Finally, Test ND {:.4f}, Test SMAPE {:.4f}, Test NRMSE {:.4f}, Test MAE {:.4f}, Test MSE {:.4f}, Test loss {:.4f}, Test ms-dtw {:.4f}"\
            .format(nd_test, smape_test, nrmse_test, mae_test, mse_test, loss_val.item(), ms_dtw_test))
